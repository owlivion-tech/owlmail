...
mod delta_sync_tests {
    ...
    async fn test_delta_sync_contacts_with_deletions() {
        ...
        // Create a test account (needed for UNIQUE(account_id, email) to work with upsert)
        let account = NewAccount {
        ...
        let acct_id = Some(db.add_account(&account).unwrap());

        // Add 5 contacts with account_id (so UNIQUE constraint triggers on upsert)
        for i in 1..=5 {
        ...
        // Change 1 contact via upsert (same account_id + email triggers ON CONFLICT)
